...
fn send_receive_encrypted_xc20p_json_test() {
    ...
    let message = Message::new() // creating message
        ...
        .as_jwe(&CryptoAlgorithm::XC20P, None) // set JOSE header for XC20P algorithm
    ...
    // Act
    let ready_to_send = message.seal(ek.as_bytes(), None).unwrap();
    let rk = bob_secret.diffie_hellman(&alice_public); // bob's shared secret calculation
    #[cfg(not(feature = "resolve"))]
    let received = Message::receive(&ready_to_send, rk.as_bytes(), None); // and now we parse received
    #[cfg(feature = "resolve")]
    ...
    assert_eq!(sample_dids::TEST_DID_SIGN_1, received.get_body().unwrap());
}
...
fn send_receive_mediated_encrypted_xc20p_json_test() {
    ...
    let message = Message::new() // creating message
        ...
        .add_header_field("another_key".into(), "another_value".into()) // another coustom header
        .kid(r#"#z6LShs9GGnqk85isEBzzshkuVWrVKsRp24GnDuHk8QWkARMW"#) // set kid header
        .routed_by(ek_to_bob.as_bytes(), "did:key:z6MknGc3ocHs3zdPiJbnaaqDi58NGb4pk1Sp9WxWufuXSdxf", None); // here we use destination key to bob and `to` header of mediator
    ...
    let ready_to_send = message
        .unwrap()
        .as_jwe(&CryptoAlgorithm::XC20P, None) // here this method call is crucial as mediator and end receiver may use different algorithms.
    ...
    #[cfg(not(feature = "resolve"))]
    let received_mediated = Message::receive(&ready_to_send.unwrap(), rk_mediator.as_bytes(), None);
    #[cfg(feature = "resolve")]
...
fn send_receive_signed_json_test() {
    ...
    // Assert
    assert!(&received.is_ok());
    assert_eq!(sample_dids::TEST_DID_SIGN_1, received.unwrap().get_body().unwrap());
}
